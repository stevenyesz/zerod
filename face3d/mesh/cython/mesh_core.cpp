...
void render_triangle(
    float* image, float* zbuffer, int* coords,
    float* t, float* v0, float* v1, float* v2,
    float intensity,
    int xrange, int yrange, int x_min, int y_min,
    int h, int w,
    int idx)
{
    int px,py;
    ...
    for(i=0;i < xrange; i++)
    {
        for (j=0; j<yrange; j++)
        {
            px = coords[(i+x_min)*h+j+y_min]; 
            py = coords[w*h+(i+x_min)*h+j+y_min];

            bx = t[3*0]*px+t[3*0+1]*py+t[3*0+2]; 
            by = t[3*1]*px+t[3*1+1]*py+t[3*1+2]; 
            bz = t[3*2]*px+t[3*2+1]*py+t[3*2+2];

            x_ = px; 
            y_ = py; 
            z_ = v0[2]*bx+v1[2]*by+v2[2]*bz;

            if (zbuffer[y_*w+x_]<z_)
            {
                for(k=0;k<3;k++)
                {
                    image[x_*h*3+y_*3+k] = intensity;
                    zbuffer[y_*h+x_] = z_;
                }
...
void _render_grid_core(
    float* image, float* t, float* vs, float* intensity, float* zbuffer, int* coords, float* vertices, int* triangles,
    float* depth_buffer,
    int nver, int ntri,
    int h, int w, int c)
{
    ...
    int tri_p0_ind, tri_p1_ind, tri_p2_ind;
    point p0, p1, p2, p;
    int x_min, x_max, y_min, y_max,xrange,yrange;
    float p_depth, p0_depth, p1_depth, p2_depth;
    ...
    for(i = 0; i < ntri; i++)
    {
        vs0 = vs+i*9; vs1 = vs+i*9+3; vs2 = vs+i*9+6;

        x_min = max((int)floor(min(vs0[0], min(vs1[0], vs2[0]))), 0);
        x_max = min((int)floor(max(vs0[0], max(vs1[0], vs2[0])))+1, w - 1);

        y_min = max((int)floor(min(vs0[1], min(vs1[1], vs2[1]))), 0);
        y_max = min((int)floor(max(vs0[1], max(vs1[1], vs2[1])))+1, h - 1);

        xrange = x_max-x_min;
        yrange = y_max-y_min;

        render_triangle(image, zbuffer, coords, t+9*i, vs0, vs1, vs2, intensity[i],xrange,yrange,x_min,y_min, h, w, i);
        //tri_p0_ind = triangles[3*i];
        //tri_p1_ind = triangles[3*i + 1];
        //tri_p2_ind = triangles[3*i + 2];
            ...
            for(x = x_min; x <= x_max; x++) //w
            {
                ...
                if(p.x < 2 || p.x > w - 3 || p.y < 2 || p.y > h - 3 || isPointInTri(p, p0, p1, p2))
                {
                    ...
                    if((p_depth > depth_buffer[y*w + x]))
                    {
                        for(k = 0; k < c; k++) // c
                        {   
                            p0_color = intensity[i];
                            p1_color = intensity[i];
                            p2_color = intensity[i]; 
