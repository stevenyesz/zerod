...
cdef extern from "mesh_core.h":
    ...
    void _render_grid_core(
        float* image, float* t, float* vs, float* intensity, float* zbuffer,int* coords,
        float* vertices, int* triangles,
        float* depth_buffer,
        int nver, int ntri,
        int h, int w, int c)
...
def render_grid_core(np.ndarray[float, ndim=3, mode = "c"] image not None,
                np.ndarray[float, ndim=3, mode = "c"] t not None,
                np.ndarray[float, ndim=3, mode = "c"] vs not None,
                np.ndarray[float, ndim=1, mode='c'] intensity not None,
                np.ndarray[float, ndim=2, mode='c'] zbuffer not None,
                np.ndarray[int, ndim=3, mode='c'] coords not None,
                np.ndarray[float, ndim=2, mode= "c"] vertices not None,
                np.ndarray[int, ndim=2, mode="c"] triangles not None,
                np.ndarray[float, ndim=2, mode = "c"] depth_buffer not None,
                int nver, int ntri,
    ...
    _render_grid_core(
        <float*> np.PyArray_DATA(image),<float*> np.PyArray_DATA(t), <float*> np.PyArray_DATA(vs),
        <float*> np.PyArray_DATA(intensity),<float*> np.PyArray_DATA(zbuffer), <int*> np.PyArray_DATA(coords),
        <float*> np.PyArray_DATA(vertices),<int*> np.PyArray_DATA(triangles),
        <float*> np.PyArray_DATA(depth_buffer),
        nver, ntri,
        h, w, c)
